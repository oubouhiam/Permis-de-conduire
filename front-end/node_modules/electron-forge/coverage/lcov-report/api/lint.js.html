...
<html lang="en">
<head>
    <title>Code coverage report for api/lint.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">api</a> lint.js
    </h1>
    <div class='clearfix'>
...
<span class="cstat-no" title="statement not covered" >      throw 'Failed to locate lintable Electron application';</span>
    }
&nbsp;
    d('executing "run lint -- --color" in dir:', dir);
    const child = yarnOrNPMSpawn(['run', 'lint', '--', '--color'], {
      stdio: process.platform === 'win32' ? <span class="branch-0 cbranch-no" title="branch not covered" >'inherit' </span>: 'pipe',
      cwd: dir,
    });
    const output = [];
    <span class="missing-if-branch" title="else path not taken" >E</span>if (process.platform !== 'win32') {
      child.stdout.on('data', data =&gt; output.push(data.toString()));
      child.stderr.on('data', <span class="fstat-no" title="function not covered" >da</span>ta =&gt; <span class="cstat-no" title="statement not covered" >output.push(data.toString().red))</span>;
    }
...
<span class="cstat-no" title="statement not covered" >          lintSpinner.fail();</span>
<span class="cstat-no" title="statement not covered" >          if (interactive) {</span>
<span class="cstat-no" title="statement not covered" >            output.forEach(<span class="fstat-no" title="function not covered" >da</span>ta =&gt; <span class="cstat-no" title="statement not covered" >process.stdout.write(data))</span>;</span>
<span class="cstat-no" title="statement not covered" >            process.exit(code);</span>
          } else {
<span class="cstat-no" title="statement not covered" >            result = output.join('');</span>
          }
        }
        resolve();
      });
...
<span class="cstat-no" title="statement not covered" >    throw result;</span>
  }
};
&nbsp;</pre></td></tr>
</table></pre>
